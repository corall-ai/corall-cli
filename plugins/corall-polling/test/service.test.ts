...
import type { HookPayload, Logger, OpenClawConfig } from "../src/types.js";
...
test("polling service forwards hook payload and acks event", async () => {
  ...
  const forwardedHooks: unknown[] = [];
  const acks: string[] = [];
  let pollCount = 0;

  const hookServer = http.createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  const eventbusServer = http.createServer(async (req: IncomingMessage, res: ServerResponse) => {
...
test("polling service deduplicates repeated delivery while acking every stream id", async () => {
  ...
  const eventbusServer = http.createServer((req: IncomingMessage, res: ServerResponse) => {
    const requestUrl = req.url;
    assert.equal(typeof requestUrl, "string");
    if (typeof requestUrl !== "string") {
      throw new Error("request URL is missing");
...
test("polling service starts after credentials add agent id", async () => {
  ...
  try {
    ...
    await fs.writeFile(
      path.join(credentialsDir, `${profile}.json`),
      JSON.stringify({ agentId: "agent-late" }),
    );
...
function testApi(): { config: OpenClawConfig; logger: Logger } {
  return {
    config: {
...
async function writeCredential(homeDir: string, profile: string, agentId: string): Promise<void> {
  const credentialsDir = path.join(homeDir, ".corall", "credentials");
  await fs.mkdir(credentialsDir, { recursive: true });
